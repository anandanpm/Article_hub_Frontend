...
const RegisterPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Reset registration success when component mounts
    dispatch(resetRegistrationSuccess())
    dispatch(clearError())
    
    // Cleanup function to reset registration success state when component unmounts
    return () => {
      dispatch(resetRegistrationSuccess())
      dispatch(clearError())
    }
  ...
  // Update form errors when server validation errors occur
  useEffect(() => {
    if (fieldErrors) {
  ...
  // Add custom CSS for error alert
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      // Check for field-specific errors and set them
      if (result.meta.requestStatus === 'rejected' && result.payload) {
        const payload = result.payload as { message: string, field?: string }
        
        if (payload.field === 'email') {
          setError('email', { type: 'server', message: payload.message })
        } else if (payload.field === 'phone') {
          setError('phone', { type: 'server', message: payload.message })
        }
      }
    } catch (err) {
      console.error("Registration error:", err)
  ...
  useEffect(() => {
    ...
    if (registrationSuccess) {
      const timer = setTimeout(() => {
        navigate("/login")
        // Reset registration success state after redirecting
        dispatch(resetRegistrationSuccess())
      }, 2000)
